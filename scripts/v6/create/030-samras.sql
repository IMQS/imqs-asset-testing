CREATE PROCEDURE [dbo].[ExportSamrasSCOAJournalRollUp]
	@batchSize INT, @assetProjectYear INT, @numberInputForms BIGINT OUTPUT, @imqsBatchId INT OUTPUT
AS
BEGIN

	DECLARE @assetProjectTable VARCHAR(16) = 'AssetProject' + STR(@assetProjectYear, 4);
	...
	DECLARE @selectSql NVARCHAR(MAX) = '
		SELECT
			...
			ap.Project_Number as C_PROJECT_CODE,
			''ASSET'' as C_ACC,
		...
		FROM
			SCOAJournal sj INNER JOIN AssetFinFormInput f ON sj.Form_Reference = f.Form_Reference INNER JOIN '+(@assetProjectTable)+' ap ON f.WIP_Project_ID = ap.Project_ID INNER JOIN AssetFinFormBatch b on SUBSTRING(f.Batch_Reference, 2, LEN(f.Batch_Reference)) = REPLACE(STR(b.BatchNr, 9),'' '', ''0'')
		WHERE
