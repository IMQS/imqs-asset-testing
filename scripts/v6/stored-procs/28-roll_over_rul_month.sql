CREATE PROCEDURE [dbo].[roll_over_rul_month](
	@FinYear INT,
...
BEGIN	
	...
	BEGIN
		SET @SQL= 'DECLARE @IntDeprDate DATETIME='+QuoteName(CAST(@IntDeprDate AS VARCHAR(MAX)),'''') + '
			...
			UPDATE '+ @FinRegister + ' 
			SET RUL=a.RUL - (12 * DeprDays/DATEDIFF(DAY, @StartDate, @EndDate))
			FROM
			(
				SELECT ComponentID
				, CostOpening
				...
				, EUL
				, RUL
				, (RUL/12)*DATEDIFF(DAY, @StartDate,@EndDate) D
				, CASE WHEN RUL<12 AND DATEADD(MONTH, RUL, @StartDate)<@IntDeprDate THEN (CarryingValueOpening-ResidualValue)
				...
				, CASE WHEN RUL<12 AND DATEADD(MONTH, RUL, @StartDate)<@IntDeprDate THEN (CarryingValueOpening-ResidualValue) / (RUL*DATEDIFF(DAY, @StartDate,@EndDate))*12
				   WHEN AdditionsFinYTD>0 THEN (AdditionsFinYTD-ResidualValue) / ((RUL/12)*DATEDIFF(DAY, @StartDate,@EndDate))
				   WHEN CostOpening>0 THEN (CarryingValueOpening-ResidualValue) /((RUL/12)*DATEDIFF(DAY, @StartDate,@EndDate))
				END DailyDepr
				, CASE WHEN DerecognitionDate>=@StartDate AND DerecognitionDate<=@IntDeprDate AND DerecognitionDate < DATEADD(DAY, (RUL/12)*DATEDIFF(DAY, @StartDate,@EndDate), @StartDate) THEN DATEDIFF(DAY,@StartDate,DerecognitionDate)
				   WHEN DerecognitionDate >=@StartDate AND RUL<12 AND DerecognitionDate<@IntDeprDate THEN (RUL/12)*DATEDIFF(DAY, @StartDate,@EndDate)
				   ...
				   WHEN RUL<12 AND DATEADD(MONTH, RUL, @StartDate)<@IntDeprDate THEN (RUL*DATEDIFF(DAY, @StartDate,@EndDate))/12
				   WHEN RUL<12 AND DATEADD(MONTH, RUL, @StartDate)>@IntDeprDate THEN DATEDIFF(DAY,@StartDate, @IntDeprDate)
				   WHEN RUL>=12 THEN DATEDIFF(DAY,@StartDate, @IntDeprDate)
				   END DeprDays
				FROM ' + @PrevRegister + ' 
