...
BEGIN
	...
	DECLARE @sql VARCHAR(MAX) = 'SELECT
				...
				dbo.convertDateToInt(GETDATE()) as TRANSACTION_DATE,
				...
				0 as CREDIT_AMOUNT,
				CASE WHEN (select BudgetLeg from SCOABudgetLeg sbl join AssetFinForm aff on sbl.FinFormNr = aff.Form_Nr where sbl.FinancialField = sj.FinancialField and sbl.FinFormNr = aff.Form_Nr) = ''CT'' then 1 else 0 END as REPLACE_WITH_DEFAULT
			FROM
			...
			SELECT
				...
				CASE WHEN (select BudgetLeg from SCOABudgetLeg sbl join AssetFinForm aff on sbl.FinFormNr = aff.Form_Nr where sbl.FinancialField = sj.FinancialField and sbl.FinFormNr = aff.Form_Nr) = ''DT'' then 1 else 0 END as REPLACE_WITH_DEFAULT
			FROM
				SCOAJournal sj '+case @depreciation when 1 then
			'INNER JOIN
				AssetRegisterIconFin'+@finYear+' f ON sj.ComponentID = f.ComponentID' else
